    ...
    for step in range(num_steps):
       ...
       < pair_threshold(is_help=True, bind_range=bind_range, help_hurt=help_hurt):
        ...
        if crossing_step is None and largest_frac >= 0.5 and step > 10:
            crossing_step = step
            crossing_pop = G.number_of_nodes()
            print("crossing step: ", crossing_step, ", crossing pop: ", crossing_pop)
...
def run_batches(num_batches, num_steps, bind_range, seed, lifespan, help_hurt):
    ...
    T = all_node_counts.shape[1] - 1
    delta = 500  # choose 500 or 1000; must be << num_steps
    t0 = max(0, T - delta)

    late_growth = (all_node_counts[:, T] - all_node_counts[:, t0]) / (T - t0)
...
def plot_scc_onset_vs_population(
    pmax_list,
    num_batches,
    num_steps,
    seed,
    lifespan,
    help_hurt,
    out_png="scc_onset_vs_population.png"
):
    """
    Plot largest SCC fraction m(t) vs population size N(t) for multiple p_max values.
    This visualizes a delayed onset of a large SCC when p_max is small.
    """
    ...
    for pmax in pmax_list:
        stats = run_batches(
            ...
            num_steps=num_steps,
            bind_range=pmax,      # <-- p_max in your paper
            seed=seed,
            lifespan=lifespan,
            help_hurt=help_hurt
        )

        N = stats["mean_node_counts"]
        m = stats["mean_largest_scc_fraction"]

        plt.plot(N, m, label=f"p_max={pmax:g}")

    plt.xlabel("Population size N(t)")
    plt.ylabel("Largest SCC fraction m(t)")
    plt.title(f"SCC onset depends on p_max (rho={help_hurt}, lifespan={lifespan})")
    plt.ylim(-0.02, 1.02)
    plt.legend()
    plt.tight_layout()
...
def plot_scc_onset_vs_time(
    ...
    lifespan,
    help_hurt,
    out_png="scc_onset_vs_time.png"
):
    """
    Optional alternative: plot m(t) vs time t for multiple p_max values.
    Use this if you want to show the onset occurs after ~500 steps directly.
    """
    plt.figure(figsize=(8, 5))
    ...
    for pmax in pmax_list:
        ...
        m = stats["mean_largest_scc_fraction"]
        plt.plot(t, m, label=f"p_max={pmax:g}")

    plt.xlabel("Time step t")
    plt.ylabel("Largest SCC fraction m(t)")
    plt.title(f"SCC onset vs time (rho={help_hurt}, lifespan={lifespan})")
    plt.ylim(-0.02, 1.02)
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_png, dpi=300)
...
def main():
    ...
    # ----- FIGURE: growth transition vs rho (full sweep 0.0 to 1.0) -----
    help_hurt_values = np.linspace(0.0, 1.0, 21)  # 21 points: 0.0, 0.05, 0.10, ... 1.0

    rhos = []
    late_growth_mu_list = []
    late_growth_se_list = []

    for rho in help_hurt_values:
        ...
        print(f"rho={rho:.3f} done in {elapsed_time:.2f}s")

        rhos.append(rho)
        late_growth_mu_list.append(stats["late_growth_mu"])
        late_growth_se_list.append(stats["late_growth_se"])

        # save diagnostic panel per rho
        plot_batch_statistics(stats)
        filename = f"rho_sweep_lifespan{lifespan}_bind{bind_range}_rho{rho:.2f}_steps{num_steps}.png"
        plt.savefig(filename, dpi=300)
        plt.close()

    # Plot growth rate vs rho
    plt.figure(figsize=(7,5))
    plt.errorbar(rhos, late_growth_mu_list, yerr=late_growth_se_list, marker="o")
    plt.axhline(0, color="k", lw=0.8)
    plt.xlabel(r"Harm-to-help ratio $\rho$")
    plt.ylabel("Late-time growth rate g")
    plt.title("Bounded vs unbounded growth transition")
    plt.tight_layout()
    plt.savefig("late_growth_vs_rho_full_sweep.png", dpi=300)
    plt.close()
